...
export class RemoveStudentFromProposalClassUseCase extends BaseManualEditUseCase {
  constructor(
  ...
  async execute(dto: RemoveStudentFromClassDTO): Promise<void> {
    const proposal = await this.proposalRepository.findById(dto.proposalId);
    if (!proposal) throw new Error('Proposal not found');

    const context = await this.getContext();
    const updatedProposal = this.manualEditor.removeStudent(
      proposal,
      dto.studentId,
      dto.classId,
      context,
