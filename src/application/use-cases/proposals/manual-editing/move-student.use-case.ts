...
export class MoveStudentBetweenProposalClassesUseCase extends BaseManualEditUseCase {
  constructor(
  ...
  async execute(dto: MoveStudentBetweenClassesDTO): Promise<void> {
    const proposal = await this.proposalRepository.findById(dto.proposalId);
    if (!proposal) throw new Error('Proposal not found');

    const context = await this.getContext();
    const updatedProposal = this.manualEditor.moveStudent(
      proposal,
      dto.studentId,
      dto.fromClassId,
      dto.toClassId,
