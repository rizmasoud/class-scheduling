...
export function EditClassDialog({ opened, onClose, classData }: Props) {
  ...
  useEffect(() => {
    if (classData) {
      reset({
        ...
        targetCapacity: classData.targetCapacity,
  ...
  const onSubmit = (values: FormValues) => {
    updateClass.mutate({
      ...
      onError: (error: any) => {
        notifications.show({ title: 'Error', message: error.message || 'Failed to update class', color: 'red' });
      },
    });
  };
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Edit Class" centered>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Stack gap="md">
          ...
          <Group justify="flex-end" mt="md">
            ...
            <Button type="submit" loading={updateClass.isPending}>Save</Button>
          </Group>
        </Stack>
      </form>
    </Modal>
